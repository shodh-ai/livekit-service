    ...
    async def processing_loop(self):
        ...
        while True:
            try:
                ...
                # If a duplicate start arrives after session is already started, skip it entirely
                if task_name == "start_tutoring_session" and getattr(self, "_session_started", False):
                    logger.info("[START] Duplicate start_tutoring_session received; session already started. Skipping.")
                ...
                if task_name == "start_tutoring_session":
                    # Mark that session has started to avoid duplicate auto-starts
                    try:
            ...
            try:
                if tool_name == "set_ui_state":
                    ...
                    if cleaned_text and self.agent_session:
                        try:
                            # This inner function defines the complete speak operation
                            async def speak_operation():
                                playback_handle = await self.agent_session.say(
                                    cleaned_text, allow_interruptions=True
                                )
                                await playback_handle  # Wait for audio to finish playing

                            # Compute a dynamic timeout based on utterance length.
                            # Approximate 2.5 words/sec + small buffer, clamped to [10s, TTS_MAX_TIMEOUT_SEC].
                            words = len(cleaned_text.split())
                            expected = (words / 2.5) + 3.0
                            import os as _os
                            try:
                                _max_tts = float(_os.getenv("TTS_MAX_TIMEOUT_SEC", "45"))
                            except Exception:
                                _max_tts = 45.0
                            timeout_s = max(10.0, min(expected, _max_tts))

                            # Run the entire operation with the computed timeout.
                            await asyncio.wait_for(speak_operation(), timeout=timeout_s)
                            logger.info(f"TTS completed for: {cleaned_text[:50]}...")

                        except asyncio.TimeoutError:
                            logger.error(f"TTS operation timed out after {timeout_s:.1f}s for: {cleaned_text[:50]}...")
                            # Best-effort attempt to stop any lingering audio
                            try:
