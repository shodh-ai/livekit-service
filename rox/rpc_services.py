...
class AgentInteractionService:
    ...
    async def TestPing(self, raw_payload: RpcInvocationData) -> str:
        ...
        try:
            # Gate auto-start on TestPing behind env flag to avoid duplicates when frontend starts explicitly
            import os as _os
            _auto_start = (_os.getenv("AUTO_START_ON_TESTPING", "false").lower() in ("1", "true", "yes"))
            if _auto_start:
                ...
                logger.info("[TestPing] AUTO_START_ON_TESTPING disabled; not queuing start")
